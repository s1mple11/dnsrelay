...
void ReceiveFromLocal()
{
	...
	if (bufferLength > 0) {
		...
		if (CurrentCacheNumber == TotalCacheNumber)
		{
			...
			//若在ip-url对照表中找到，查询该域名对应的ip是否是0.0.0.0，如果是则将数据包首部设置为不回答，如果否将首部设置为回答
			else {
				...
				//将修改好的报文发送回客户端
				bufferLength = sendto(localSocket, sendbuf, currenLength, 0, (SOCKADDR*)& client, sizeof(client)); 
				//打印调试信息
				if (debugLevel >= 1)
				{
					if (IPV4) {
		...
		//若在cache中找到，查询该域名对应的ip是否是0.0.0.0，如果是则将数据包首部设置为不回答，如果否将首部设置为回答
		else {
			...
			//将修改好的报文发送回客户端
			bufferLength = sendto(localSocket, sendbuf, currentLength, 0, (SOCKADDR*)& client, sizeof(client)); 
			//打印调试信息
			if (debugLevel >= 1)
			{
				if (IPV4) {
